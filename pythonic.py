...
for i in range(len(b)):
	k.append(loadtxt('/Users/etrott12/Dropbox/emery/axionCAMB/mp/wb%s_m%s.dat' %(a[i],b[i]), usecols = [0]))
	Pk.append(loadtxt('/Users/etrott12/Dropbox/emery/axionCAMB/mp/wb%s_m%s.dat' %(a[i],b[i]), usecols = [1]))
Pk = np.asarray(Pk).reshape(len(a),len(Pk[i]))
k = np.asarray(k).reshape(len(a),len(k[i]))
...
#	return param_deriv
#wb_deriv = fin_deriv(k,Pk,fin_epsilon[0],central_values[0])
