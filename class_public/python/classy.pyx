...
cdef class Class:
    ...
    # def fix_dmeff_cns(self):
    #     for k,v in self._pars.items():
    #         m = re.search('cn_(\di?)_(\d+)', k)
    #         if m:
    #             i = m.group(1)
    #             j = int(m.group(2))
    #             key = 'cn_{}_dmeff'.format(i)
    #             if key not in self._dmeff_pars:
    #                 self._dmeff_pars[key] = [0]*15
    #             self._dmeff_pars[key][j] = v

    #     for k,v in self._dmeff_pars.items():
    #         vstring = ''
    #         for v1 in v:
    #             vstring += ',{}'.format(v1)
    #         self._pars[k] = vstring[1:]
    ...
    # Defined twice ?
    def Omega_m(self):
        return self.ba.Omega0_b+self.ba.Omega0_cdm+self.ba.Omega0_dbk+self.ba.Omega0_dmeff+self.ba.Omega0_ncdm_tot + self.ba.Omega0_dcdm
    ...
    def dmeff_sigma(self):
        """
        Return dmeff transfer cross section for hydrogen and helium,
        along with the velocity dependence.
        """
        cdef int index_sig
        cdef int sig_size = self.th.dmeff_sig_size

        cdef np.ndarray[DTYPE_t, ndim=1] sig_H    = np.zeros((sig_size), 'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] sig_He   = np.zeros((sig_size), 'float64')
        cdef np.ndarray[DTYPE_t, ndim=1] sig_vpow = np.zeros((sig_size), 'float64')

        for index_sig in xrange(sig_size):
            sig_H[index_sig]    = self.th.dmeff_sig_H[index_sig]
            sig_He[index_sig]   = self.th.dmeff_sig_He[index_sig]
            sig_vpow[index_sig] = self.th.dmeff_sig_vpow[index_sig]

        return sig_H,sig_He,sig_vpow
        ...
        for name in names:
            if name == 'h':
                ...
                value = (self.ba.Omega0_b + self.ba.Omega0_cdm+
                         self.ba.Omega0_dbk + self.ba.Omega0_dmeff+
                ...
                value = self.ba.sigM_dbk * self.ba.m_dbk
            elif name == 'z_dbk_decoupling':
                value = self.ba.z_dbk_decoupling
            elif name == 'm_dmeff':
                value = self.ba.m_dmeff
            elif name == 'z_dmeff_decoupling':
                value = self.th.z_dmeff_decoupling
            elif name == 'dmeff_sigma_p':
                value = self.th.dmeff_sigma_p
            elif name == 'tau_reio':
