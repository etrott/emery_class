...
int thermodynamics_init(
  ...
  class_test((pth->annihilation>0)&&(pba->has_cdm==_FALSE_)&&(pba->has_dbk==_FALSE_)&&(pba->has_dmeff==_FALSE_),
             pth->error_message,
  ...
  class_test((pth->decay>0)&&(pba->has_cdm==_FALSE_)&&(pba->has_dbk==_FALSE_)&&(pba->has_dmeff==_FALSE_),
             pth->error_message,
...
int thermodynamics_onthespot_energy_injection(
  ...
  rho_cdm_today = pow(pba->H0*_c_/_Mpc_over_m_,2)*3/8./_PI_/_G_*(pba->Omega0_cdm+pba->Omega0_dbk+pba->Omega0_dmeff)*_c_*_c_; /* energy density in J/m^3 */
...
int thermodynamics_energy_injection(
  ...
  if (preco->annihilation > 0) {
    ...
    if (preco->has_on_the_spot == _FALSE_) {
      ...
      factor = _sigma_ * nH0 / pba->H0 * _Mpc_over_m_ / sqrt(pba->Omega0_b+pba->Omega0_cdm+pba->Omega0_dbk+pba->Omega0_dmeff);
...
int thermodynamics_recombination_with_hyrec(
  ...
  param.omh2 = (pba->Omega0_b+pba->Omega0_cdm+pba->Omega0_dbk+pba->Omega0_dmeff+pba->Omega0_ncdm_tot)*pba->h*pba->h;
...
int thermodynamics_dmeff_init(struct background *pba,
  ...
  int index_elem, index_resp, index_type, index_sig;
  int index_ti, index_tau, index;

  /* Initialize z_dmeff_decoupling to something unphysical. */
  ...
  /* Initialize flag to indicate if there are nonzero interactions. */
  pdmeff->has_nonzero = _TRUE_;

  /* Define indices for each baryonic element. */
  index_elem = 0;
  class_define_index(pdmeff->index_H1, _TRUE_,index_elem,1); // H-1
  class_define_index(pdmeff->index_He4,_TRUE_,index_elem,1); // He-4
  pdmeff->size_elem = index_elem;

  /* Define indices for each type of response function. */
  index_resp = 0;
  class_define_index(pdmeff->index_M,   _TRUE_,index_resp,1); // M
  class_define_index(pdmeff->index_Ppp, _TRUE_,index_resp,1); // Phi''
  class_define_index(pdmeff->index_PppM,_TRUE_,index_resp,1); // Phi'' M
  class_define_index(pdmeff->index_tPp, _TRUE_,index_resp,1); // tildePhi'
  class_define_index(pdmeff->index_Spp, _TRUE_,index_resp,1); // Sigma''
  class_define_index(pdmeff->index_Sp,  _TRUE_,index_resp,1); // Sigma'
  class_define_index(pdmeff->index_D,   _TRUE_,index_resp,1); // Delta
  class_define_index(pdmeff->index_DSp, _TRUE_,index_resp,1); // Delta Sigma'
  pdmeff->size_resp = index_resp;

  /* Define indices for saved quantities in R_table. */
  index_type = 0;
  class_define_index(pdmeff->index_R_op1,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_R_op2,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_R_vp2,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_R_qm2,_TRUE_,index_type,1);
  pdmeff->size_R_type = index_type;

  /* Define indices for saved quantities in W_table. */
  index_type = 0;
  class_define_index(pdmeff->index_W_iso1,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_W_iso2,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_W_ypow,_TRUE_,index_type,1);
  class_define_index(pdmeff->index_W_resp,_TRUE_,index_type,1);
  pdmeff->size_W_type = index_type;
  ...
  /* Allocate memory. */
  class_alloc(pdmeff->bmuT,sizeof(double)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->mass,sizeof(double)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->spin,sizeof(double)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->frac,sizeof(double)*pdmeff->size_elem,pth->error_message);

  class_calloc(pdmeff->size_R,pdmeff->size_resp,sizeof(int),pth->error_message);
  class_calloc(pdmeff->size_W,pdmeff->size_elem,sizeof(int),pth->error_message);

  class_alloc(pdmeff->R,   sizeof(double*)*pdmeff->size_resp,pth->error_message);
  class_alloc(pdmeff->R_table,sizeof(int*)*pdmeff->size_resp,pth->error_message);

  class_alloc(pdmeff->W,   sizeof(double*)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->W_table,sizeof(int*)*pdmeff->size_elem,pth->error_message);

  class_alloc(pdmeff->sig,    sizeof(double*)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->sig_v2,    sizeof(int*)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->sig_vperp2,sizeof(int*)*pdmeff->size_elem,pth->error_message);
  class_alloc(pdmeff->size_sig,   sizeof(int)*pdmeff->size_elem,pth->error_message);
  ...
  /* Fill table for coupling coefficients. */
  class_call(dmeff_cc(pba,pdmeff),pdmeff->error_message,pth->error_message);

  /* Fill tables for response functions. */
  class_call(dmeff_response(pba,pth,pdmeff),pdmeff->error_message,pth->error_message);

  /* Calculate transfer cross section. */
  class_call(dmeff_cross_section(pba,pth,pdmeff),pdmeff->error_message,pth->error_message);

  /* Save transfer cross sections for convenience. */
  // Find range of possible v2 powers.
  int v2_max, v2_min, v2_idx;
  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){
    for(index_sig=0; index_sig < pdmeff->size_sig[index_elem]; index_sig++){
      if(index_sig==0 && index_elem==0){
        v2_max = pdmeff->sig_v2[index_elem][index_sig];
        v2_min = pdmeff->sig_v2[index_elem][index_sig];
      } else{
        v2_max = MAX(v2_max,pdmeff->sig_v2[index_elem][index_sig]);
        v2_min = MIN(v2_min,pdmeff->sig_v2[index_elem][index_sig]);
      }
    }
  }

  pth->dmeff_sig_size = v2_max - v2_min + 1;
  class_alloc(pth->dmeff_sig_H,   pth->dmeff_sig_size*sizeof(double),pth->error_message);
  class_alloc(pth->dmeff_sig_He,  pth->dmeff_sig_size*sizeof(double),pth->error_message);
  class_alloc(pth->dmeff_sig_vpow,pth->dmeff_sig_size*sizeof(int),   pth->error_message);

  // Initialize.
  for(index_sig=0; index_sig < pth->dmeff_sig_size; index_sig++){
    pth->dmeff_sig_H[index_sig]  = 0.0;
    pth->dmeff_sig_He[index_sig] = 0.0;
    pth->dmeff_sig_vpow[index_sig] = 2*(v2_min + index_sig);
  }

  // Fill.
  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){
    for(index_sig=0; index_sig < pdmeff->size_sig[index_elem]; index_sig++){
      v2_idx = v2_min + pdmeff->sig_v2[index_elem][index_sig];
      if(index_elem == pdmeff->index_H1){
        pth->dmeff_sig_H[v2_idx]  += pdmeff->sig[index_elem][index_sig];
      } else if(index_elem == pdmeff->index_He4){
        pth->dmeff_sig_He[v2_idx] += pdmeff->sig[index_elem][index_sig];
      }
    }
  }
...
int thermodynamics_dmeff_free(struct dmeff_thermo *pdmeff){
  int index_elem, index_resp;

  /* Allocated in dmeff_cross_section(). */
  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){
    free(pdmeff->sig_v2[index_elem]);
    free(pdmeff->sig_vperp2[index_elem]);
    free(pdmeff->sig[index_elem]);
  }

  /* Allocated in dmeff_response(). */
  for(index_resp=0; index_resp < pdmeff->size_resp; index_resp++){
    free(pdmeff->R[index_resp]);
    free(pdmeff->R_table[index_resp]);
  }

  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){
    free(pdmeff->W[index_elem]);
    free(pdmeff->W_table[index_elem]);
  }

  /* Allocated in dmeff_cc(). */
  free(pdmeff->cc_qm2);
  free(pdmeff->cc);
  ...
  free(pdmeff->size_sig);
  free(pdmeff->sig_v2);
  free(pdmeff->sig_vperp2);
  free(pdmeff->sig);
...
/**
 *  This function restructures the input coupling coefficients into
 *  a compact table.
 *
 * @param pba    Input:        background structure
 * @param pdmeff Input/Output: dmeff_thermo structure
...
int dmeff_cc(struct background *pba,
             struct dmeff_thermo *pdmeff){

  int index_cc, index_iso, index_op, i;
  int cc_qm2_min, cc_qm2_max;

  /* Determine minimum and maximum (q/mN)^2 dependence of any coupling coefficient. */
  for(i=0; i < pba->cc_dmeff_num; i++){
    if(i==0){
      cc_qm2_min = pba->cc_dmeff_qm2[i];
      cc_qm2_max = pba->cc_dmeff_qm2[i];
      continue;
    }
    cc_qm2_min = MIN(cc_qm2_min,pba->cc_dmeff_qm2[i]);
    cc_qm2_max = MAX(cc_qm2_min,pba->cc_dmeff_qm2[i]);
  }

  /* Allocate memory. */
  pdmeff->size_cc = cc_qm2_max - cc_qm2_min + 1;
  class_alloc(pdmeff->cc_qm2,sizeof(int)*pdmeff->size_cc,pdmeff->error_message);
  class_alloc(pdmeff->cc,sizeof(double)*pdmeff->size_cc*2*_OP_DMEFF_,pdmeff->error_message);
  for(index_cc=0; index_cc < pdmeff->size_cc; index_cc++){
    pdmeff->cc_qm2[index_cc] = cc_qm2_min + index_cc;
  }

  /* Initialize. */
  for(i=0; i < pdmeff->size_cc*2*_OP_DMEFF_; i++){
    pdmeff->cc[i] = 0.;
  }

  /* Set nonzero coupling coefficients */
  for(i=0; i<pba->cc_dmeff_num; i++){
    index_op = pba->cc_dmeff_op[i] - 1;
    index_cc = pba->cc_dmeff_qm2[i] - pdmeff->cc_qm2[0];
    index_iso = 0;
    pdmeff->cc[(index_cc*_OP_DMEFF_ + index_op)*2 + index_iso] +=
      (pba->cc_dmeff_p[i] + pba->cc_dmeff_n[i]) / 2.;
    index_iso = 1;
    pdmeff->cc[(index_cc*_OP_DMEFF_ + index_op)*2 + index_iso] +=
      (pba->cc_dmeff_p[i] - pba->cc_dmeff_n[i]) / 2.;
  }
...
/**
 *  Find defined response index based on common name.
 *
 * @param pdmeff Input:  dmeff_thermo structure
 * @param name   Input:  name of response
 * @param index  Output: index of response
 */

int dmeff_get_response(struct dmeff_thermo *pdmeff, char *name, int *index){
  if(      strcmp(name,"M"   )==0 ) { *index = pdmeff->index_M;    }
  else if( strcmp(name,"Ppp" )==0 ) { *index = pdmeff->index_Ppp;  }
  else if( strcmp(name,"PppM")==0 ) { *index = pdmeff->index_PppM; }
  else if( strcmp(name,"tPp" )==0 ) { *index = pdmeff->index_tPp;  }
  else if( strcmp(name,"Spp" )==0 ) { *index = pdmeff->index_Spp;  }
  else if( strcmp(name,"Sp"  )==0 ) { *index = pdmeff->index_Sp;   }
  else if( strcmp(name,"D"   )==0 ) { *index = pdmeff->index_D;    }
  else if( strcmp(name,"DSp" )==0 ) { *index = pdmeff->index_DSp;  }
 ...
 * @param pth    Input: thermodynamics structure
 * @param pdmeff Input:  dmeff_thermo structure
 * @param Z      Input:  atomic number
 * @param A      Input:  atomic mass
 * @param index  Output: index of response
 */

int dmeff_get_element(struct background *pba,
                      struct thermo *pth,
                      struct dmeff_thermo *pdmeff,
                      int Z, int A, int *index){

  int index_elem;
  double mass,b;

  /* Set element index. */
  if(      (Z==1) && (A==1) ) { index_elem = pdmeff->index_H1;  }
  else if( (Z==2) && (A==4) ) { index_elem = pdmeff->index_He4; }
  else { return _FAILURE_; }

  /* Set element mass. */
  if(index_elem == pdmeff->index_He4) { mass = _m_H_*_not4_; }
  else { mass = _m_H_ * (double)A; }
  pdmeff->mass[index_elem] = mass;

  /* Set element fraction (fraction of rho_baryon). */
  if(index_elem == pdmeff->index_H1)  { pdmeff->frac[index_elem] = (1.-pth->YHe); }
  else if(index_elem == pdmeff->index_He4 && pth->use_helium_dmeff==_TRUE_) { pdmeff->frac[index_elem] = pth->YHe; }
  else { pdmeff->frac[index_elem] = 0.0; }

  /* Set nuclear size scale (harmonic oscillator parameter in shell model expansion).
     Convert from fm to kg^-1 to make b*mu_T dimensionless. */
  b = sqrt(41.467 / (45.*pow((double)A,-1./3.) - 25.*pow((double)A,-2./3.)));
  b *= 1.0e-15 / (_h_P_ / _TWOPI_ / _c_); // convert from fm to kg^-1

  pdmeff->bmuT[index_elem] = b * (mass * pba->m_dmeff)/(mass + pba->m_dmeff);
...
/**
 *  This function reads in and saves DM and nuclear response functions
 *  from external files.
 *
 *  For the nuclear response W (see 1501.03729):
 *    The form of each term is assumed to be W0 * y^ypow * e^(-2y),
 *    where y=(qb/2) and W0 is a numerical coefficient.
 *    Only for hydrogen does the factor of e^(-2y) not appear.
 *
 *    Here, we save W0*4*pi/(2*JN+1), where JN is the nucleon spin
 *    (additionally, the file itself gives W0*4*pi and not W0).
 *
 *  For the dark matter response R (see 1308.6288):
 *    The form of each term is assumed to be
 *    R0 * c * c' * (v_perp^2)^alpha * ((q/mN)^2)^beta,
 *    where c and c' are the coupling coefficients of two EFT operators.
 *
 *    Here, we omit the couplings (and any associate powers of q/mN)
 *    and simply save the associated operator labels. The couplings
 *    are incorporated later when calculating the cross section.
 *
 *  NB: The papers 1501.03729 and 1308.6288 relate the (proton,neutron)
 *    basis to the isospin basis differently, so we multiply W by 4
 *    to stay consistent with the notation in 1308.6288.
 *
 * @param pba    Input:        background structure
 * @param pth    Input:        thermodynamics structure
 * @param pdmeff Input/Output: dmeff_thermo structure
...
int dmeff_response(struct background *pba,
                   struct thermo *pth,
                   struct dmeff_thermo *pdmeff){

  int index_elem,index_resp,index_W,index_R;
  short need_size_resp;

  FILE *fresp, *fnucl;
  int Z, A, entries, iso1, iso2, ypow, op1, op2, vp2, qm2, J2_flag;
  double JN, Wval, Rnum, Rden;
  char line[_LINE_LENGTH_MAX_],resp[_LINE_LENGTH_MAX_];
  FileName DM_file, nucl_file;

  /* Open files that hold response function information. */
  sprintf(DM_file,__CLASSDIR__);
  strcat(DM_file,"/dmeff/DM_response.dat");

  sprintf(nucl_file,__CLASSDIR__);
  strcat(nucl_file,"/dmeff/nuclear_response_HO.dat");

  class_open(fnucl,nucl_file,"r",pdmeff->error_message);
  class_open(fresp,DM_file,  "r",pdmeff->error_message);

  /* Fill DM response functions. */
  need_size_resp = _TRUE_;
  while(fgets(line,_LINE_LENGTH_MAX_,fresp) != NULL) {
    if( line[0]=='#' ) continue;
    if(need_size_resp == _TRUE_){
      class_test(sscanf(line,"%s%d",resp,&entries) != 2,
                 pdmeff->error_message,"DMeff: Could not read in size of DM response function.");
      class_call(dmeff_get_response(pdmeff,resp,&index_resp),
                 pdmeff->error_message,"DMeff: Unrecognized response function.");
      /* Allocate memory. */
      class_alloc(pdmeff->R[index_resp],sizeof(double)*entries,
                  pdmeff->error_message);
      class_alloc(pdmeff->R_table[index_resp],sizeof(int)*entries*pdmeff->size_R_type,
                  pdmeff->error_message);
      need_size_resp = _FALSE_;
      pdmeff->size_R[index_resp] = 0;
    } else{
      class_test(sscanf(line,"%d%d%d%d%d%lf%lf",&op1,&op2,&vp2,&qm2,&J2_flag,&Rnum,&Rden) != 7,
                 pdmeff->error_message,"DMeff: Could not read in entry for DM response function.");
      index_R = pdmeff->size_R[index_resp];
      pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_op1] = op1;
      pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_op2] = op2;
      pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_vp2] = vp2;
      pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_qm2] = qm2;
      pdmeff->R[index_resp][index_R] = Rnum/Rden;
      if(J2_flag==1) {
        pdmeff->R[index_resp][index_R] *= pba->spin_dmeff * (pba->spin_dmeff + 1.);
      }
      pdmeff->size_R[index_resp]++;
      if(pdmeff->size_R[index_resp] == entries) { need_size_resp = _TRUE_; }
    }
  }

  /* Fill nuclear response functions. */
  need_size_resp = _TRUE_;
  while(fgets(line,_LINE_LENGTH_MAX_,fnucl) != NULL) {
    if( line[0]=='#' ) continue;
    if(need_size_resp == _TRUE_){
      class_test(sscanf(line,"%d%d%lf%d",&Z,&A,&JN,&entries) != 4,
                 pdmeff->error_message,"DMeff: Could not read in size of nuclear response function.");
      class_call(dmeff_get_element(pba,pth,pdmeff,Z,A,&index_elem),
                 pdmeff->error_message,"DMeff: Unrecognized element.");
      pdmeff->spin[index_elem] = JN;
      /* Allocate memory. */
      class_alloc(pdmeff->W[index_elem],sizeof(double)*entries,
                  pdmeff->error_message);
      class_alloc(pdmeff->W_table[index_elem],sizeof(int)*entries*pdmeff->size_W_type,
                  pdmeff->error_message);
      need_size_resp = _FALSE_;
      pdmeff->size_W[index_elem] = 0;
    } else{
      class_test(sscanf(line,"%s%d%d%d%lf",resp,&iso1,&iso2,&ypow,&Wval) != 5,
                 pdmeff->error_message,"DMeff: Could not read in entry for nuclear response function.");
      class_call(dmeff_get_response(pdmeff,resp,&index_resp),
                 pdmeff->error_message,pdmeff->error_message);

      index_W = pdmeff->size_W[index_elem];
      pdmeff->W_table[index_elem][index_W * pdmeff->size_W_type + pdmeff->index_W_iso1] = iso1;
      pdmeff->W_table[index_elem][index_W * pdmeff->size_W_type + pdmeff->index_W_iso2] = iso2;
      pdmeff->W_table[index_elem][index_W * pdmeff->size_W_type + pdmeff->index_W_ypow] = ypow;
      pdmeff->W_table[index_elem][index_W * pdmeff->size_W_type + pdmeff->index_W_resp] = index_resp;
      pdmeff->W[index_elem][index_W] = 4. * Wval/(2.*JN+1.);

      pdmeff->size_W[index_elem]++;
      if(pdmeff->size_W[index_elem] == entries) { need_size_resp = _TRUE_; }
    }
  }
...
/**
 *  Calculate transfer cross section for dark matter scattering with nuclei.
 *  For elements other than hydrogen, the cross section saved here does not
 *  include the dimensionless integral (related to the angular integration
 *  of the differential cross section). It is easier to handle this part
 *  with velocity averaging in thermodynamics_dmeff_rate().
 *
 * @param pba    Input: background structure
...
int dmeff_cross_section(struct background *pba,
                        struct thermo *pth,
                        struct dmeff_thermo *pdmeff){

  double convert, vev_sq, mu_T, m_N;
  double Msq, prefactor, cc1, cc2;
  double int_factor, int_factor_MT;

  int iso1, iso2, ypow, op1, op2, vp2, qm2;
  int qm2_total, index_vp2, j;
  int index_elem, index_resp, index_cc1, index_cc2;
  int index_W, index_R, index_sig;

  /* Constants. */
  m_N = _m_p_; // mass of proton (nucleon)
  convert = _h_P_ / _TWOPI_ / _c_; // convert kg^-1 to m
  vev_sq = 1./(2.*1.1663787e-5); // square of the Higgs VEV (weak scale) in GeV^2
  vev_sq *= pow(1.0e9 * _eV_ / (_c_ * _c_), 2); // convert to kg^2

  /* Save actual cross section (all other cross sections in dmeff are actually
     transfer cross sections) for scattering with a proton (hydrogen) at v=220 km/s. */
  pth->dmeff_sigma_p = 0.0;

  /* Calculate cross section for each element. */
  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){
    /* Estimate number of terms and allocate memory. */
    index_sig = 0;
    for(index_W=0; index_W < pdmeff->size_W[index_elem]; index_W++){
      index_resp = pdmeff->W_table[index_elem][index_W*pdmeff->size_W_type + pdmeff->index_W_resp];
      for(index_R=0; index_R < pdmeff->size_R[index_resp]; index_R++){
        index_sig += 1;
      }
    }
    index_sig *= pdmeff->size_cc;
    class_alloc(pdmeff->sig[index_elem],sizeof(double)*index_sig,pdmeff->error_message);
    class_alloc(pdmeff->sig_v2[index_elem],sizeof(int)*index_sig,pdmeff->error_message);
    class_alloc(pdmeff->sig_vperp2[index_elem],sizeof(int)*index_sig,pdmeff->error_message);
    index_sig = 0; // reset

    /* Set more constants. */
    mu_T = (pba->m_dmeff * pdmeff->mass[index_elem]) / (pba->m_dmeff + pdmeff->mass[index_elem]);
    prefactor = (2./_PI_) * pow( convert * pba->cc_dmeff_scale * mu_T  / vev_sq, 2. );

    /* Loop over all terms in W. */
    for(index_W=0; index_W < pdmeff->size_W[index_elem]; index_W++){
      index_resp = pdmeff->W_table[index_elem][index_W*pdmeff->size_W_type + pdmeff->index_W_resp];
      iso1 = pdmeff->W_table[index_elem][index_W*pdmeff->size_W_type + pdmeff->index_W_iso1];
      iso2 = pdmeff->W_table[index_elem][index_W*pdmeff->size_W_type + pdmeff->index_W_iso2];
      ypow = pdmeff->W_table[index_elem][index_W*pdmeff->size_W_type + pdmeff->index_W_ypow];
      if(pdmeff->W[index_elem][index_W]==0.0) { continue; }

      /* For each term in W, find contribution from R. */
      for(index_R=0; index_R < pdmeff->size_R[index_resp]; index_R++){
        op1 = pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_op1];
        op2 = pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_op2];
        vp2 = pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_vp2];
        qm2 = pdmeff->R_table[index_resp][index_R * pdmeff->size_R_type + pdmeff->index_R_qm2];
        if(pdmeff->R[index_resp][index_R]==0.0) { continue; }

        /* Incorporate (q/mN)^2 appearing in front of certain responses in matrix element. */
        if((index_resp==pdmeff->index_Ppp) || (index_resp==pdmeff->index_PppM) ||
           (index_resp==pdmeff->index_tPp) || (index_resp==pdmeff->index_D) ||
           (index_resp==pdmeff->index_DSp)) {
          qm2 += 1;
        }

        /* Incorporate coupling coefficients. */
        for(index_cc1=0; index_cc1<pdmeff->size_cc; index_cc1++){
          cc1 = pdmeff->cc[(index_cc1*_OP_DMEFF_ + (op1-1))*2 + iso1];
          if(cc1==0.0) { continue; }

          for(index_cc2=0; index_cc2<pdmeff->size_cc; index_cc2++){
            cc2 = pdmeff->cc[(index_cc2*_OP_DMEFF_ + (op2-1))*2 + iso2];
            if(cc2==0.0) { continue; }

            qm2_total = qm2 + pdmeff->cc_qm2[index_cc1] + pdmeff->cc_qm2[index_cc2];

            // Contribution from response functions
            Msq = cc1 * cc2 * pdmeff->R[index_resp][index_R] * pdmeff->W[index_elem][index_W];

            // Contribution from writing (q/mN)^2 = (x/xm) * v^2 * (2*mu_T/mN)^2
            Msq *= pow(2.*mu_T/m_N, 2*qm2_total);

            // Contribution from writing y = (x/xm) * v^2 * (b*mu_T)^2
            Msq *= pow(pdmeff->bmuT[index_elem], 2*ypow);

            // Contribution from integrating differential cross section
            // (hydrogen only -- other elements must include thermal terms)
            if(index_elem == pdmeff->index_H1){
              for(index_vp2=0; index_vp2 <= vp2; index_vp2++){
                int_factor    = 1./(1. + index_vp2 + qm2_total + ypow);
                int_factor_MT = 1./(2. + index_vp2 + qm2_total + ypow);
                /* Include binomial coefficient. */
                if((index_vp2>0) && (index_vp2<vp2)){
                  for(j=0; j < index_vp2; j++){
                    int_factor    *= ( (double)(vp2-j) )/( (double)(j+1) );
                    int_factor_MT *= ( (double)(vp2-j) )/( (double)(j+1) );
                  }
                }
              }
              pth->dmeff_sigma_p += prefactor/2. * Msq * int_factor * pow(0.22/_c_,2*(vp2+qm2_total+ypow));
              Msq *= int_factor_MT;
            }

            pdmeff->sig[index_elem][index_sig] = prefactor * Msq;
            pdmeff->sig_v2[index_elem][index_sig] = vp2 + qm2_total + ypow;
            pdmeff->sig_vperp2[index_elem][index_sig] = vp2;
            index_sig++;
          }
        }
      }
    ...
    /* Catch case in which there are no nonzero terms. */
    if(index_sig==0){
      pdmeff->has_nonzero = _FALSE_;
      pdmeff->sig[index_elem][index_sig]        = 0.0;
      pdmeff->sig_v2[index_elem][index_sig]     = 0;
      pdmeff->sig_vperp2[index_elem][index_sig] = 0;
      index_sig++;
    }

    pdmeff->size_sig[index_elem]   = index_sig;
    pdmeff->sig[index_elem]        = realloc(pdmeff->sig[index_elem],sizeof(double)*index_sig);
    pdmeff->sig_v2[index_elem]     = realloc(pdmeff->sig_v2[index_elem],sizeof(int)*index_sig);
    pdmeff->sig_vperp2[index_elem] = realloc(pdmeff->sig_vperp2[index_elem],sizeof(int)*index_sig);
  }
 ...
 *  but it is overwritten if use_temperature_dmeff == _FALSE_.
 *
 * @param pba        Input:        background structure
 * @param pth        Input:        thermodynamics structure
 * @param pdmeff     Input:        dmeff_thermo structure
...
int thermodynamics_dmeff_rate(struct background *pba,
  ...
  int index_elem, index_sig, index_vp2, vp2, j;
  double z, a, H, rho_baryon, Tb, Tdmeff, dTdtau_T;
  double m_T, mu_T, sig, cn, vpow, rate;
  double avg_term, avg_p_1, avg_m_1;
  ...
  if(pdmeff->has_nonzero == _FALSE_){
    pvecthermo[pth->index_th_Tdmeff] = 0.0;
  } else if(pth->use_temperature_dmeff == _FALSE_){
    pvecthermo[pth->index_th_Tdmeff] = pba->T_cmb / a;
  }
  ...
  /* Terms in the cross section are grouped by powers of velocity, and
     we must treat each term seperately when performing velocity averaging. */
  for(index_elem=0; index_elem < pdmeff->size_elem; index_elem++){

    m_T = pdmeff->mass[index_elem];
    mu_T = (pba->m_dmeff * m_T) / (pba->m_dmeff + m_T);

    for(index_sig=0; index_sig < pdmeff->size_sig[index_elem]; index_sig++){
      vpow = 2.0 * (double)(pdmeff->sig_v2[index_elem][index_sig]);
      vp2  = pdmeff->sig_vperp2[index_elem][index_sig];
      cn = pow(2.,(vpow + 5.)/2.) * tgamma(3. + vpow/2.) / (3.*_SQRT_PI_);
      sig = pdmeff->sig[index_elem][index_sig];

      rate = a * pdmeff->frac[index_elem] * rho_baryon * cn * sig/(pba->m_dmeff + m_T);

      /* Include internal degrees of freedom from phase space distribution. */
      rate *= (2.*pba->spin_dmeff + 1.) * (2.*pdmeff->spin[index_elem] + 1.);

      /* Thermal contribution. */
      avg_term = (Tb/m_T + Tdmeff/pba->m_dmeff)*_k_B_/(_c_*_c_);
      rate *= pow(avg_term, (vpow + 1.)/2.);

      /* For elements other than hydrogen, incorporate thermally averaged
         contribution from e^(-2y) in nuclear response. */
      if(index_elem != pdmeff->index_H1){
        avg_p_1  = 1. + pow(2.*pdmeff->bmuT[index_elem],2.)*avg_term;
        avg_m_1  = 1. - pow(2.*pdmeff->bmuT[index_elem],2.)*avg_term;
        for(index_vp2=0; index_vp2 <= vp2; index_vp2++){
          rate *= 1./(2. + vpow/2.+(double)(index_vp2 - vp2));
          rate *= pow(avg_m_1, index_vp2) * pow(avg_p_1, -(2.+vpow/2.+(double)(index_vp2 - vp2)));
          /* Include binomial coefficient. */
          if((index_vp2>0) && (index_vp2<vp2)){
            for(j=0; j < index_vp2; j++){
              rate *= ( (double)(vp2-j) )/( (double)(j+1) );
            }
          }
        }
      }
      ...
      pvecthermo[pth->index_th_dkappaT_dmeff] += rate * pba->m_dmeff / (pba->m_dmeff + m_T);
  ...
  if(pdmeff->has_nonzero == _FALSE_){
    ...
    dTdtau_T = -a*H;
  } else if(z > pth->z_dmeff_decoupling){
    dTdtau_T = -a*H;
  } else{
...
int thermodynamics_dmeff_temperature(struct precision *ppr,
  ...
  for(index_tau=0; index_tau < pba->bt_size-1; index_tau++){
    ...
    if(pth->z_dmeff_decoupling < 0.0){
      a = pvecback[pba->index_bg_a];
      H = pvecback[pba->index_bg_H];
      if(a*H > pvecthermo[pth->index_th_dkappaT_dmeff]/10.){
