...
int input_read_parameters(
  ...
  /* Omega_0_dbk (dbk) */
  class_call(parser_read_double(pfc,"Omega_dbk",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_dbk",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_dbk or omega_dbk, choose one");
  if (flag1 == _TRUE_)
    pba->Omega0_dbk = param1;
  if (flag2 == _TRUE_)
    pba->Omega0_dbk = param2/pba->h/pba->h;

  Omega_tot += pba->Omega0_dbk;

  /* m_dbk (dbk) in kg */
  class_call(parser_read_double(pfc,"m_dbk",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_)
    pba->m_dbk = param1 * 1.0e9 * _eV_ / (_c_ * _c_); // convert GeV to kg

  /* npow_dbk (dbk) */
  class_read_double("npow_dbk",pba->npow_dbk);
  /* cannot have even numbers <= -6 due to non-analyticity in gamma function 
     when calculating c_n coefficients in background.c */
  npow_mod = pba->npow_dbk - 2.*floor(pba->npow_dbk/2.);
  class_test((pba->npow_dbk <= -6. && npow_mod == 0), errmsg, "In input file, you cannot set npow_dbk to an even integer less than or equal to -6.");

  /* sigM_dbk (dbk) in m^2 / kg */
  class_call(parser_read_double(pfc,"sigM_dbk",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  if (flag1 == _TRUE_)
    pba->sigM_dbk = param1 / 10.; // convert cm^2/g to m^2/kg
  ...
  if(pba->Omega0_dmeff != 0.){
    ...
    /* cc_dmeff_scale */
    class_read_double("cc_dmeff_scale",pba->cc_dmeff_scale);

    /* cc_dmeff_num */
    class_read_int("cc_dmeff_num",pba->cc_dmeff_num);
    class_test(pba->cc_dmeff_num < 1,
               errmsg,
               "If running with dmeff, there must be at least one nonzero coupling.");

    /* cc_dmeff_op */
    class_read_list_of_integers("cc_dmeff_op", pba->cc_dmeff_op, pba->cc_dmeff_num);
    for(i=0; i<pba->cc_dmeff_num; i++){
      class_test(pba->cc_dmeff_op[i] < 1, errmsg,
                 "DMeff operator labels begin at 1.");
      class_test(pba->cc_dmeff_op[i] > _OP_DMEFF_, errmsg,
                 "DMeff operator labels end at %d.",_OP_DMEFF_);
    }

    /* cc_dmeff_qm2 */
    class_read_list_of_integers("cc_dmeff_qm2", pba->cc_dmeff_qm2, pba->cc_dmeff_num);
    for(i=0; i<pba->cc_dmeff_num; i++){
      class_test(pba->cc_dmeff_qm2[i] < 0, errmsg,
                 "DMeff requires nonzero powers of (q/m_N)^2 at this time.");
    }

    /* cc_dmeff_p */
    class_read_list_of_doubles("cc_dmeff_p", pba->cc_dmeff_p, pba->cc_dmeff_num);

    /* cc_dmeff_n */
    class_read_list_of_doubles("cc_dmeff_n", pba->cc_dmeff_n, pba->cc_dmeff_num);
    ...
    if (flag1 == _TRUE_){
      if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
        pth->use_helium_dmeff = _TRUE_;
      }
      else{
        pth->use_helium_dmeff = _FALSE_;
      }
    }

    /* use_temperature_dmeff */
    class_call(parser_read_string(pfc,
                                  "use_temperature_dmeff",
                                  &string1,
                                  &flag1,
    ...
    if (flag1 == _TRUE_){
      if((strstr(string1,"y") != NULL) || (strstr(string1,"Y") != NULL)){
        pth->use_temperature_dmeff = _TRUE_;
      }
      else{
        pth->use_temperature_dmeff = _FALSE_;
      }
    }
  }
...
int input_default_params(
  ...
  pba->Omega0_dbk = 0.0;
  pba->m_dbk = 1.0 * 1.0e9 * _eV_ / (_c_ * _c_);
  pba->npow_dbk = 0;
  pba->sigM_dbk = 0.12 / 10.;
  pba->Omega0_dmeff = 0.0;
  pba->m_dmeff = 1.0 * 1.0e9 * _eV_ / (_c_ * _c_);
  pba->spin_dmeff = 0.5;
  pba->cc_dmeff_scale = 1.0;
  pba->cc_dmeff_num = 0;
  pba->cc_dmeff_op  = NULL;
  pba->cc_dmeff_qm2 = NULL;
  pba->cc_dmeff_p   = NULL;
  pba->cc_dmeff_n   = NULL;
  ...
  pba->K = 0.;
  pba->sgnK = 0;
  pba->Omega0_lambda = 1.-pba->Omega0_k-pba->Omega0_g-pba->Omega0_ur-pba->Omega0_b-pba->Omega0_cdm-pba->Omega0_ncdm_tot-pba->Omega0_dcdmdr-pba->Omega0_dbk-pba->Omega0_dmeff;
  pba->Omega0_fld = 0.;
  pba->a_today = 1.;
  pba->w0_fld=-1.;
  pba->wa_fld=0.;
...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case theta_s:
      ...
      break;
    case Omega_dcdmdr:
      Omega_M = ba.Omega0_cdm+ba.Omega0_dbk+ba.Omega0_dmeff+ba.Omega0_dcdmdr+ba.Omega0_b;
  ...
  * - Version 3: use attractor solution */
    ...
    case Omega_ini_dcdm:
      ...
      Omega_M = ba.Omega0_cdm+ba.Omega0_dbk+ba.Omega0_dmeff+Omega0_dcdmdr+ba.Omega0_b;
      gamma = ba.Gamma_dcdm/ba.H0;
      if (gamma < 1)
        a_decay = 1.0;
      else
