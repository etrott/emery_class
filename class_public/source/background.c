...
int background_functions(
  ...
  /* dbk quantities */
  double F_He = 0.76; // conservative estimate from Cora
  double cn, rho_baryon, avg_term, Tb;
  double f_He = 0.24; // helium mass fraction
  ...
  if (pba->has_dbk == _TRUE_) {
    pvecback[pba->index_bg_rho_dbk] = pba->Omega0_dbk * pow(pba->H0,2) / pow(a_rel,3);
    rho_tot += pvecback[pba->index_bg_rho_dbk];
    p_tot += 0.;
    rho_m += pvecback[pba->index_bg_rho_dbk];

    // temperature, dkappa, and c2 would normally be calculated in thermodynamics.c, but dbk relies only on background quantities
    Tb = pba->T_cmb / a_rel;
    pvecback[pba->index_bg_Tdbk] = Tb;

    cn = pow(2.,(pba->npow_dbk + 5.)/2.) * tgamma(3. + pba->npow_dbk / 2.) / (3.*sqrt(_PI_));
    rho_baryon = pvecback[pba->index_bg_rho_b]*3.*_c_*_c_/(8.*_PI_*_G_*_Mpc_over_m_); // baryon (includes He) density in kg / m^2 / Mpc
    avg_term = pow((Tb/_m_H_ + pvecback[pba->index_bg_Tdbk]/pba->m_dbk)*_k_B_/(_c_*_c_),(pba->npow_dbk + 1.)/2.); // from velocity averaging

    pvecback[pba->index_bg_dkappa_dbk] = a*rho_baryon*cn*pba->sigM_dbk*pba->m_dbk/(pba->m_dbk + _m_H_)*avg_term*F_He; // in 1/Mpc

    pvecback[pba->index_bg_cdbk2] = 0;
  }
  ...
  if (pba->has_dmeff == _TRUE_) {
    pvecback[pba->index_bg_Tdmeff] = Tb;
...
int background_init(
  ...
  /* computing z_dbk_decoupling */
  double F_He = 0.76; // conservative estimate from Cora
  double cn, rho_baryon, avg_term, z_dec=0.0;

  if(pba->has_dbk == _TRUE_){
    cn = pow(2.,(pba->npow_dbk + 5.)/2.) * tgamma(3. + pba->npow_dbk / 2.) / (3.*sqrt(_PI_));

    rho_baryon = pba->Omega0_b*pba->H0*3.*_c_*_c_/(8.*_PI_*_G_*_Mpc_over_m_); // baryon (includes He) density in kg / m^2 / Mpc
    avg_term = pba->T_cmb*(1./_m_H_ + 1./pba->m_dbk)*_k_B_/(_c_*_c_); // from velocity averaging

    pba->z_dbk_decoupling = pow(sqrt(pba->Omega0_g)/rho_baryon*3.*_m_H_/pba->m_dbk * (_m_H_ + pba->m_dbk)/pba->m_dbk / (cn*pba->sigM_dbk*F_He),2./(pba->npow_dbk+3.)) * pow(avg_term,-(pba->npow_dbk+1.)/(pba->npow_dbk+3.)) - 1.;
...
int background_free_input(
  ...
  if (pba->Omega0_dmeff != 0.){
    free(pba->cc_dmeff_op);
    free(pba->cc_dmeff_qm2);
    free(pba->cc_dmeff_p);
    free(pba->cc_dmeff_n);
  }
...
int background_indices(
  ...
  /* - index for rho_dbk */
  class_define_index(pba->index_bg_rho_dbk,pba->has_dbk,index_bg,1);

  /* - index for Tdbk */
  class_define_index(pba->index_bg_Tdbk,pba->has_dbk,index_bg,1);

  /* - index for dkappa/dtau for dbk */
  class_define_index(pba->index_bg_dkappa_dbk,pba->has_dbk,index_bg,1);

  /* - index for c2 for dbk */
  class_define_index(pba->index_bg_cdbk2,pba->has_dbk,index_bg,1);
...
int background_output_titles(struct background * pba,
  ...
  class_store_columntitle(titles,"(.)rho_dbk",pba->has_dbk);
  class_store_columntitle(titles,"T_dbk",pba->has_dbk);
  class_store_columntitle(titles,"kappa' (dbk) [Mpc^-1]",pba->has_dbk);
  class_store_columntitle(titles,"c_dbk^2",pba->has_dbk);
...
int background_output_data(
  ...
  for (index_tau=0; index_tau<pba->bt_size; index_tau++){
    ...
    class_store_double(dataptr,pvecback[pba->index_bg_rho_dbk],pba->has_dbk,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_Tdbk],pba->has_dbk,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_dkappa_dbk],pba->has_dbk,storeidx);
    class_store_double(dataptr,pvecback[pba->index_bg_cdbk2],pba->has_dbk,storeidx);
