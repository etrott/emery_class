...
int spectra_indices(
  ...
  class_define_index(psp->index_tr_delta_dbk,ppt->has_source_delta_dbk,index_tr,1);
  ...
  class_define_index(psp->index_tr_theta_dbk,ppt->has_source_theta_dbk,index_tr,1);
    ...
    for (index_k=0; index_k<psp->ln_k_size; index_k++) {
      ...
      for (index_ic = 0; index_ic < psp->ic_size[index_md]; index_ic++) {
        ...
        if (pba->has_dbk == _TRUE_) {
          ...
          rho_i = pvecback_sp_long[pba->index_bg_rho_dbk];

          if (ppt->has_source_delta_dbk == _TRUE_) {

            delta_i = ppt->sources[index_md]
              [index_ic * ppt->tp_size[index_md] + ppt->index_tp_delta_dbk]
              [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

            psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_delta_dbk] = delta_i;
          ...
          if (ppt->has_source_theta_dbk == _TRUE_) {
            ...
            theta_i = ppt->sources[index_md]
              [index_ic * ppt->tp_size[index_md] + ppt->index_tp_theta_dbk]
              [(index_tau-psp->ln_tau_size+ppt->tau_size) * ppt->k_size[index_md] + index_k];

            psp->matter_transfer[((index_tau*psp->ln_k_size + index_k) * psp->ic_size[index_md] + index_ic) * psp->tr_size + psp->index_tr_theta_dbk] = theta_i;
...
int spectra_output_tk_titles(struct background *pba,
  ...
  if (output_format == class_format) {
    ...
    if (ppt->has_density_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"d_dbk",pba->has_dbk);
    ...
    if (ppt->has_velocity_transfers == _TRUE_) {
      ...
      class_store_columntitle(titles,"t_dbk",((pba->has_dbk == _TRUE_) && (ppt->gauge != synchronous)));
      class_store_columntitle(titles,"t_dmeff",((pba->has_dmeff == _TRUE_) && (ppt->gauge != synchronous)));
    ...
    class_store_columntitle(titles,"-T_dbk/k2",_TRUE_);
      ...
      for (index_k=0; index_k<psp->ln_k_size; index_k++) {
        ...
        if (output_format == class_format) {
          ...
          if (ppt->has_density_transfers == _TRUE_) {
            ...
            class_store_double(dataptr,tk[psp->index_tr_delta_dbk],ppt->has_source_delta_dbk,storeidx);
          ...
          if (ppt->has_velocity_transfers == _TRUE_) {
            ...
            class_store_double(dataptr,tk[psp->index_tr_theta_dbk],ppt->has_source_theta_dbk,storeidx);
          ...
          class_store_double_or_default(dataptr,-tk[psp->index_tr_delta_dbk]/k2,ppt->has_source_delta_dbk,storeidx,0.0);
